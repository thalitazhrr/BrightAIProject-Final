  ...
  SQL_QUERY: `
    WITH HSI_LOYALTY_BASE AS (
        SELECT 
            ...
            CASE
                WHEN CAST(LOS AS NUMBER) >= 60 AND COALESCE(TREMS_REV_P, TREMS_REV_REF, 0) >= 300000 AND NVL(ADDON_TOTAL, 0) >= 2 THEN 'RISIKO_RENDAH'
                WHEN CAST(LOS AS NUMBER) >= 36 AND COALESCE(TREMS_REV_P, TREMS_REV_REF, 0) >= 200000 THEN 'RISIKO_MENENGAH_RENDAH'
                WHEN CAST(LOS AS NUMBER) >= 24 AND COALESCE(TREMS_REV_P, TREMS_REV_REF, 0) >= 150000 THEN 'RISIKO_MENENGAH'
            ...
            CASE
                WHEN CAST(LOS AS NUMBER) >= 60 AND COALESCE(TREMS_REV_P, TREMS_REV_REF, 0) >= 500000 THEN 'PELANGGAN_LOYAL_PREMIUM'
                WHEN CAST(LOS AS NUMBER) >= 36 AND COALESCE(TREMS_REV_P, TREMS_REV_REF, 0) >= 300000 THEN 'PELANGGAN_LOYAL_TINGGI'
                WHEN CAST(LOS AS NUMBER) >= 24 AND COALESCE(TREMS_REV_P, TREMS_REV_REF, 0) >= 200000 THEN 'PELANGGAN_LOYAL_SEDANG'
                WHEN CAST(LOS AS NUMBER) >= 12 AND COALESCE(TREMS_REV_P, TREMS_REV_REF, 0) >= 150000 THEN 'PELANGGAN_LOYAL_STANDAR'
