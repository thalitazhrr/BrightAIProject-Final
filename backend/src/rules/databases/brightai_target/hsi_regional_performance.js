    ...
    REGIONAL_MARKET_SHARE AS (
        SELECT t.PERIODE,
            ...
            ROUND(REGIONAL_TARGET * 100.0 / 
                SUM(REGIONAL_TARGET) OVER (PARTITION BY PERIODE, LLEVEL, SEGMEN, SATUAN), 2
    ...
    REGIONAL_TRENDS AS (
        SELECT t.PERIODE,
            ...
            CASE 
                ...
                WHEN ((REGIONAL_REALISASI - PREV_REALISASI) * 100.0 / PREV_REALISASI) >= 15 THEN 'MOMENTUM_TINGGI'
                WHEN ((REGIONAL_REALISASI - PREV_REALISASI) * 100.0 / PREV_REALISASI) >= 5 THEN 'MOMENTUM_SEDANG'
                WHEN ((REGIONAL_REALISASI - PREV_REALISASI) * 100.0 / PREV_REALISASI) >= -5 THEN 'MOMENTUM_STABIL'
