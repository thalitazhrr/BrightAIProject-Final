...
\begin{enumerate}
  \item Keterbatasan akses analitik berbasis percakapan
  ...
  \item Ketiadaan fitur prediksi yang mudah diakses pengguna nonteknis
  ...
  \item Minimnya integrasi \textit{rule-based query dan \textit{natural language generation}}
...
\begin{enumerate}
  ...
  \item Mengintegrasikan model peramalan deret waktu (\textit{time series forecasting}) terpilih ke dalam sistem dengan membandingkan model \textit{deep learning} (GRU dan LSTM) terhadap \textit{baseline} statistik (ARIMA dan \textit{Prophet}) untuk menghasilkan prediksi terhadap indikator bisnis pelanggan, seperti volume pesanan, pencapaian target penjualan, volume \textit{churn} pelanggan agregat bulanan, dan kualitas layanan operasional. Prediksi \textit{churn} dalam konteks ini merujuk pada peramalan jumlah total pelanggan yang berhenti berlangganan per periode, bukan klasifikasi biner per pelanggan individual.
  ...
  \item Merancang dan mengimplementasikan antarmuka percakapan berbasis web yang mendukung interaksi bahasa alami dan alur terpandu (\textit{guided flow}) bagi pengguna internal.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Sistem menggunakan metode peramalan deret waktu yang mencakup model \textit{deep learning} berupa LSTM\label{abbr:LSTM} (\textit{Long Short-Term Memory}) dan GRU\label{abbr:GRU} (\textit{Gated Recurrent Unit}), serta model statistik berupa ARIMA\label{abbr:ARIMA} (\textit{Autoregressive Integrated Moving Average}) dan Prophet sebagai \textit{baseline} pembanding. Perbandingan antara model statistik dan model \textit{deep learning} dilakukan untuk menentukan pendekatan yang paling tepat sesuai dengan karakteristik data. Model TFT\label{abbr:TFT} (\textit{Temporal Fusion Transformer}) \textbf{tidak termasuk} dalam cakupan implementasi penelitian ini; TFT hanya dikaji dalam studi literatur sebagai rekomendasi pengembangan di masa mendatang.
  ...
  \item Sistem dievaluasi melalui pengujian fungsional, pengukuran akurasi prediksi, analisis waktu respons, serta pengukuran kepuasan perwakilan divisi untuk pengguna internal. Evaluasi klasifikasi difokuskan pada level pencocokan aturan (\textit{rule matching}) terhadap 38 kelas aturan bisnis, bukan pada level klasifikasi \textit{intent} (7+1 kategori) karena taksonomi \textit{intent} dirancang bersifat saling eksklusif secara semantik sehingga klasifikasi pada level tersebut bersifat deterministik.
...
\begin{enumerate}[label=\alph*.]
  \item Observasi sistem yang ada, yaitu pengamatan terhadap sistem yang saat ini digunakan organisasi untuk mengidentifikasi keterbatasan dan kebutuhan pengguna internal.
  
  \item Wawancara dengan pemangku kepentingan, dilakukan secara terstruktur dengan perwakilan pemangku kepentingan internal untuk menggali kebutuhan fungsional dan ekspektasi terhadap sistem baru.
  
  \item Analisis data historis, dilakukan untuk menelaah volume dan tren permintaan, pola musiman, serta kelengkapan dan kualitas data yang tersedia.
  
  \item Dokumentasi temuan, mengompilasi seluruh hasil observasi, wawancara, dan analisis dalam bentuk laporan yang akan dilampirkan.
\end{enumerate}
...
\begin{enumerate}[label=\alph*.]
  \item Pencarian literatur sistematis pada basis data akademik (IEEE Xplore, ScienceDirect, SpringerLink, ACM Digital Library, Google Scholar) menggunakan kata kunci terkait \textit{business intelligence}, \textit{chatbot}, \textit{natural language generation}, dan \textit{time series forecasting}. 
  
  \item Pengelompokan literatur berdasarkan tema utama, yaitu konsep \textit{Business Intelligence}, teknologi \textit{chatbot} dan \textit{conversational} AI, pemrosesan bahasa alami, metode peramalan deret waktu, integrasi sistem, dan studi kasus implementasi.
  
  \item Penapisan dan analisis kualitas literatur berdasarkan faktor dampak, jumlah sitasi, metodologi, dan relevansi terhadap penelitian.
  
  \item Sintesis literatur untuk membangun kerangka teoretis, mengidentifikasi praktik terbaik dan teknologi terkini, serta menyusun tinjauan pustaka pada Bab II.
  
  \item Dokumentasi proses dalam bentuk peninjauan penelitian terdahulu yang ditemukan dan membuat celah penelitian dan kontribusi besar yang ditawarkan penelitian ini.
\end{enumerate}
...
\begin{enumerate}[label=\alph*.]
  \item Perancangan arsitektur sistem, yaitu penyusunan arsitektur berlapis yang mengintegrasikan \textit{chatbot}, pemrosesan bahasa alami, mesin \textit{rule-based query}, dan peramalan deret waktu, serta penentuan teknologi dan kerangka kerja yang akan digunakan.
  
  \item Perancangan modul \textit{chatbot}, meliputi komponen pemahaman bahasa alami (\textit{Natural Language Understanding}), manajemen dialog (\textit{Dialogue Management}), pembangkitan bahasa alami (\textit{Natural Language Generation}), serta pendefinisian \textit{intent} dan entitas yang akan dikenali.
  
  \item Pengimplementasian \textit{rule-based query}, yaitu pendefinisian aturan untuk kueri terstruktur dan berulang serta pemetaan antara \textit{intent} dengan templat kueri.
  
  \item Perancangan model peramalan deret waktu, meliputi penentuan variabel target, perancangan praolah data, pemilihan arsitektur model (ARIMA, Prophet, LSTM, dan GRU), perbandingan model statistik dengan model \textit{deep learning}, dan strategi evaluasi model.
  
  \item Perancangan basis data dan gudang data, yaitu perancangan skema basis data untuk menyimpan data pelanggan, hasil prediksi, dan data pelatihan NLP, serta perancangan proses ETL\label{abbr:ETL} (\textit{Extract, Transform, Load}).
  
  \item Perancangan antarmuka pengguna, meliputi pembuatan \textit{wireframe} dan \textit{mockup} untuk antarmuka \textit{chatbot}.
\end{enumerate}
...
\begin{enumerate}[label=\alph*.]
  \item Pengembangan \textit{backend}, yaitu implementasi mesin \textit{rule-based query}, model NLP untuk klasifikasi \textit{intent} dan pengenalan entitas, model peramalan deret waktu, serta penyediaan API\label{abbr:API} \textit{backend}.
  
  \item Pengembangan \textit{frontend}, meliputi implementasi antarmuka \textit{chatbot}.
  
  \item Pengembangan basis data, yaitu implementasi skema basis data, proses ETL untuk integrasi data, dan \textit{data pipeline} untuk pemrosesan data.
  
  \item Integrasi komponen, meliputi integrasi \textit{chatbot} dengan mesin \textit{rule-based query} dan NLP, integrasi model peramalan dengan sistem BI, serta implementasi mekanisme tembolok (\textit{caching}) untuk performa.
\end{enumerate}
...
\begin{enumerate}[label=\alph*.]
  \item Evaluasi pencocokan aturan (\textit{rule matching)}, yaitu evaluasi menggunakan \textit{confusion matrix} serta menghitung nilai akurasi, presisi, daya ingat (\textit{recall}), dan skor-F1 (\textit{F1-score}) per kelas yang kemudian diagregasi secara makro (\textit{macro}) dan mikro (\textit{micro}) pada sampel kueri uji. Turut dilakukan pula analisis nilai keyakinan (\textit{confidence score}) dan tingkat cakupan (\textit{coverage rate}).
  \item Evaluasi model peramalan deret waktu, yaitu evaluasi menggunakan protokol \textit{walk-\-forward va\-li\-da\-tion} dengan \textit{seasonal naive} sebagai garis dasar. Kinerja model diukur menggunakan \textit{Mean Absolute Error} (MAE)\label{abbr:MAE}, \textit{Root Mean Squared Error} (RMSE)\label{abbr:RMSE}, \textit{symmetric Mean Absolute Percentage Error} (sMAPE)\label{abbr:sMAPE}, dan \textit{Mean Absolute Scaled Error} (MASE)\label{abbr:MASE}, dilengkapi \textit{skill score} terhadap garis dasar untuk kategorisasi status kelulusan model.
  \item Evaluasi kinerja implementasi \textit{Natural Language Generation (NLG)}, yaitu evaluasi terhadap teks yang dihasilkan sistem yang mencakup dua aspek utama: akurasi faktual terhadap nilai perhitungan matematis pada \textit{backend}, serta evaluasi subjektif berbasis pengguna (\textit{human evaluation}) untuk menilai kelengkapan informasi, kemudahan pemahaman, kesesuaian narasi, dan kualitas tata bahasa.
\end{enumerate}
...
\begin{enumerate}[label=\alph*.]
  \item Dokumentasi teknis, yaitu penyusunan dokumentasi API, dokumentasi kode, panduan pengguna (\textit{user manual}), dan panduan penyebaran (\textit{deployment guide}).
  
  \item Penyusunan laporan tugas akhir, meliputi penulisan laporan sesuai format yang ditetapkan dengan penyuntingan dan pemeriksaan kelengkapan.
  
  \item Persiapan presentasi, meliputi pembuatan \textit{slide} presentasi, penyusunan skenario demonstrasi sistem, dan kurasi materi pendukung.
  
  \item Revisi dan finalisasi, yaitu revisi berdasarkan masukan pembimbing, finalisasi laporan dan sistem, serta persiapan sidang tugas akhir.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Bab II Studi Literatur: Berisi kajian pustaka yang relevan dengan topik tugas akhir, mencakup teori dan konsep \textit{Business Intelligence}, \textit{chatbot} berbasis aturan, pemrosesan bahasa alami, pembangkitan bahasa alami, serta metode peramalan deret waktu.
  \item Bab III Analisis Masalah: Berisi analisis kondisi sistem \textit{Business Intelligence} saat ini, identifikasi masalah pengguna, kebutuhan fungsional dan nonfungsional sistem, serta analisis pemilihan solusi teknologi yang paling tepat.
  \item Bab IV Perancangan Sistem \textit{Business Intelligence Chatbot}: Berisi perancangan arsitektur sistem, perancangan layanan antarmuka percakapan (klasifikasi \textit{intent} dan mesin aturan), model peramalan deret waktu, modul pembangkitan bahasa alami (NLG), rancangan antarmuka pengguna, dan rancangan basis data.
  \item Bab V Implementasi Sistem \textit{Business Intelligence Chatbot}: Berisi penjelasan tentang proses implementasi komponen sistem, termasuk sistem \textit{frontend}, \textit{backend}, layanan \textit{machine learning} (peramalan deret waktu), klasifikasi maksud pengguna, mesin aturan, pembangkitan bahasa alami, serta keamanan sistem.
  \item Bab VI Evaluasi: Berisi hasil pengujian fungsional sistem, evaluasi performa modul pencocokan aturan (\textit{rule matching}), evaluasi kinerja model peramalan deret waktu, serta evaluasi kualitas pembangkitan bahasa alami.
  ...
  \item Lampiran: Berisi dokumen pendukung seperti hasil wawancara, transkrip formulir evaluasi, dan tautan repositori kode sumber.
\end{enumerate}
