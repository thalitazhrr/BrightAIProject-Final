...
\hline
\textbf{Domain} & \textbf{Kelengkapan (\%)} & \textbf{\textit{Missing}} (\%) & \textbf{Duplikat} & \textbf{Anomali} \\
\hline
...
Sistem peramalan dalam penelitian ini mengadopsi pendekatan bertahap yang dimulai dari model statistik sebagai \textit{baseline}, kemudian dibandingkan dengan model berbasis pembelajaran mendalam (\textit{deep learning}). Evaluasi perbandingan ini bertujuan untuk memastikan bahwa kompleksitas tambahan model \textit{deep learning} memberikan peningkatan akurasi yang signifikan dan terukur. Sistem menggunakan empat pendekatan model yang dikategorikan sebagai berikut: (1) model statistik \textit{baseline} berupa ARIMA dan Prophet, dan (2) model \textit{deep learning} utama berupa LSTM (\textit{Long Short-Term Memory}), GRU (\textit{Gated Recurrent Unit}), dan TFT (\textit{Temporal Fusion Transformer}).
...
Tabel~\ref{tab:perbandingan-model} menyajikan perbandingan karakteristik seluruh model peramalan yang digunakan dalam penelitian ini, mencakup model statistik \textit{baseline} (ARIMA/SARIMA dan Prophet) serta model \textit{deep learning} utama (LSTM, GRU, TFT).
...
\hline
\textbf{Aspek Perbandingan} & \textbf{ARIMA} & \textbf{Prophet} & \textbf{LSTM} & \textbf{GRU} & \textbf{TFT} \\
\hline
...
\hline
Interpretabilitas & Sangat Tinggi & Tinggi & Rendah & Rendah & Tinggi (\textit{attention}) \\
\hline
Kemampuan Multivariat & Terbatas & Terbatas & Baik & Baik & Sangat Baik \\
\hline
Kebutuhan Volume Data & Rendah & Rendah & Sedang & Sedang & Tinggi \\
\hline
Peran dalam Penelitian & \textit{Baseline} & \textit{Baseline} & Model Utama & Model Utama & Opsi Lanjutan \\
\hline
\end{tabular}
\end{table}
