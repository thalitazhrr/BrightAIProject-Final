...
\item Ketergantungan pada Tim Teknis
...
\item Kurangnya Kemampuan Prediktif
...
\item Akses yang Tidak Fleksibel
...
\item Ketiadaan Antarmuka Percakapan
...
\item Keterlambatan Respons terhadap Pertanyaan Bisnis
...
\item Kesulitan Mengakses Analisis Berdasarkan Kebutuhan Khusus
...
\item Ketiadaan Kemampuan Prediksi Otomatis
...
\item Akses yang Tidak Ramah Pengguna Nonteknis
...
\item Fragmentasi Akses Wawasan
...
& & \textbf{Keluaran}: Jika nilai keyakinan $\geq$ 40 (skala 0--100): kueri diteruskan ke mesin kueri berbasis aturan. Jika nilai keyakinan $<$ 40: sistem memicu permintaan klarifikasi ke pengguna atau menawarkan alternatif \textit{intent}. \\
\hline
...
& & \textbf{Keluaran}: Nilai prediksi dan selang kepercayaan. \\
\hline
...
NF-3 & Performa dan Waktu Respons & Sistem harus mampu memberikan waktu respons rata-rata 15 detik untuk 95\% dari kueri tipikal. Dalam kueri kompleks yang memerlukan peramalan atau analisis data besar, waktu respons rata-rata adalah 45 detik. Sistem harus dapat memproses minimal 100 kueri per menit pada kondisi beban normal dan hingga 1000 kueri per jam pada kondisi beban puncak tanpa penurunan performa yang signifikan. \\
\hline
...
Berikut adalah empat alternatif solusi utama yang telah diidentifikasi dan dianalisis berdasarkan studi literatur, praktik industri, dan kebutuhan spesifik organisasi, sebagaimana disajikan pada Tabel~\ref{tab:alternatif-solusi}:
...
Mengimplementasikan sistem BI \textit{chatbot} yang terintegrasi dengan tiga komponen, yaitu klasifikasi maksud berbasis pola dengan pemberian nilai keyakinan, pembangkitan bahasa alami berbasis templat untuk respons yang beragam namun aman, dan model peramalan deret waktu untuk prediksi otomatis. & 
...
Tabel~\ref{tab:bsc-solusi1} menyajikan hasil analisis untuk alternatif Solusi 1 berdasarkan empat perspektif \textit{Balanced Scorecard}. Pendekatan ini mengandalkan pencocokan pola statis tanpa kemampuan pemahaman konteks yang dinamis.
...
Tabel~\ref{tab:bsc-solusi2} memaparkan evaluasi untuk Solusi 2 yang mengombinasikan kecerdasan buatan, pemrosesan bahasa alami, dan model prediktif. Solusi ini menjadi alternatif komprehensif dalam menjawab kebutuhan analitik otomatis.
...
Tabel~\ref{tab:bsc-solusi3} menunjukkan penilaian \textit{Balanced Scorecard} untuk Solusi 3, yaitu peningkatan visual dan interaktivitas pada dasbor yang sudah ada. Pendekatan ini mempertahankan paradigma konvensional dalam penyajian wawasan bisnis.
...
Tabel~\ref{tab:bsc-solusi4} merangkum analisis untuk Solusi 4 yang sepenuhnya mengandalkan model bahasa besar pra-latih. Pendekatan ini menawarkan kewajaran bahasa yang sangat tinggi namun membawa tantangan dalam hal determinisme dan infrastruktur.

\begin{longtable}{|p{3.5cm}|p{9cm}|}
\caption{Analisis \textit{Balanced Scorecard} untuk LLM\label{abbr:LLM}}
...
Dalam memberikan gambaran yang lebih komprehensif, Tabel~\ref{tab:matriks-bsc} menyajikan matriks perbandingan keempat solusi berdasarkan keempat perspektif \textit{Balanced Scorecard}, beserta karakteristik pengguna yang paling sesuai untuk masing-masing solusi:
...
\begin{enumerate}
    \item Keselarasan dengan Kebutuhan
    ...
    \item Skalabilitas dan Adaptabilitas
    ...
    \item Tahan Masa Depan dan Platform untuk Inovasi
    ...
    \item Pengembalian Investasi dan Justifikasi Finansial
    ...
    \item Keunggulan Kompetitif dan Diferensiasi
    ...
    \item Adopsi Pengguna dan Kepuasan
    
    Antarmuka percakapan berbasis bahasa wajar memiliki tingkat adopsi yang jauh lebih tinggi dibandingkan dasbor tradisional karena lebih intuitif dan tidak memerlukan pelatihan ekstensif. Penelitian menunjukkan bahwa \textit{chatbot} berbasis AI mampu menangani sebagian besar pertanyaan rutin secara otomatis, yang secara signifikan mengurangi beban kerja tim dan meningkatkan kepuasan pengguna \autocite{Adam2021Chatbots}.
    
    \item Keamanan dan Kontrol Data
    ...
    \item Pembelajaran dan Pertumbuhan Organisasi
...
Tahap pemahaman data (\textit{data understanding}) merupakan fase kedua dalam metodologi CRISP-DM yang bertujuan untuk mengumpulkan data awal, mengidentifikasi karakteristik data, mendeteksi permasalahan kualitas data, serta menemukan wawasan awal yang dapat membentuk hipotesis terhadap informasi tersembunyi dalam data. Pada konteks pengembangan sistem \textit{Business Intelligence} berbasis \textit{chatbot}, tahap ini berfokus pada eksplorasi komprehensif terhadap lima domain data utama yang tersimpan dalam skema \texttt{DWH\_MOIS} pada \textit{Enterprise Data Warehouse}.

\subsection{Sumber Data dan Arsitektur Penyimpanan}

Sistem peramalan dan analisis performa berbasis \textit{chatbot} terhubung secara langsung dengan sumber data utama berupa \textit{Enterprise Data Warehouse} (EDW)\label{abbr:EDW} yang menggunakan basis data Oracle pada server dengan skema \texttt{DWH\_MOIS}. Koneksi yang diatur menggunakan metode \textit{connection pooling} serta \textit{parameter binding} untuk menjamin efisiensi dan keamanan saat eksekusi kueri. Skema \texttt{DWH\_MOIS} berisi lima tabel domain utama dengan prefiks \texttt{BRIGHTAI\_} yang masing-masing memiliki fokus bisnis dan struktur data berbeda.

\subsubsection{Asal-Usul Data dan Izin Pemakaian}

Seluruh data yang digunakan dalam penelitian ini merupakan data operasional internal perusahaan yang tersimpan pada \textit{Enterprise Data Warehouse} milik PT Telkom Indonesia (Persero) Tbk, khususnya pada skema \texttt{DWH\_MOIS}. Akses terhadap data diperoleh melalui mekanisme resmi kerja praktik yang diselenggarakan di Divisi \textit{Digital Business} and Technology pada periode kerja praktik penulis. Penggunaan data untuk keperluan penelitian dan pengembangan prototipe sistem telah mendapat persetujuan dari pembimbing lapangan selaku perwakilan pihak perusahaan.

Data yang diproses oleh sistem bersifat agregat dan tidak mengandung informasi identitas pribadi pelanggan secara langsung. Atribut pelanggan seperti \texttt{NCLI}, \texttt{ND}, dan \texttt{NOTEL} yang terdapat dalam tabel sumber digunakan semata-mata sebagai kunci penghitungan agregasi (misalnya \texttt{COUNT(DISTINCT NCLI)} untuk menghitung jumlah pelanggan unik) dan tidak ditampilkan maupun disimpan dalam keluaran sistem. Seluruh respons \textit{chatbot} dan hasil peramalan disajikan dalam bentuk metrik agregat pada level wilayah (regional, witel) dan periode waktu (bulanan) sehingga tidak memungkinkan identifikasi pelanggan individual. Sistem juga dirancang dengan akses baca saja (\textit{read-only}) terhadap basis data, tanpa kemampuan untuk mengubah, menghapus, atau mengekspor data mentah dari \textit{data warehouse}.

\begin{table}[htbp]
\centering
\caption{Ringkasan Domain Data pada Skema \texttt{DWH\_MOIS}}
...
\hline
Tabel & \texttt{DWH\_MOIS.BRIGHTAI\_SALES} \\
\hline
...
Pemetaan parameter penjualan pada tabel di atas berfungsi mengisolasi informasi performa aktivasi layanan secara akurat.

\subsubsection{Domain REVENUE}
...
\textbf{Komponen} & \textbf{Deskripsi dan Atribut} \\
\hline
Tabel & \texttt{DWH\_MOIS.BRIGHTAI\_REVENUE} \\
\hline
...
Struktur \textit{revenue} yang dipaparkan dalam tabel ini memfasilitasi pelacakan pertumbuhan moneter berbasis waktu.
...
\textbf{Komponen} & \textbf{Deskripsi dan Atribut} \\
\hline
Tabel & \texttt{DWH\_MOIS.BRIGHTAI\_DAPROS} \\
\hline
...
Melalui atribut segmentasi pelanggan tersebut, manajer wilayah dapat membedah penyebaran konsumen dan status kepemilikan jaringan.
...
\textbf{Komponen} & \textbf{Deskripsi dan Atribut} \\
\hline
Tabel & \texttt{DWH\_MOIS.BRIGHTAI\_TARGET} \\
\hline
...
Metrik capaian target di atas digunakan sebagai tolok ukur utama guna memastikan bahwa ekspansi bisnis tetap berada di jalur operasional yang direncanakan.
...
\textbf{Komponen} & \textbf{Deskripsi dan Atribut} \\
\hline
Tabel & \texttt{DWH\_MOIS.BRIGHTAI\_CT0\_NAL} \\
\hline
...
\hline
Metrik \textit{Churn} & \texttt{CHURN\_RATE}, \texttt{CHURN\_DATE}, \texttt{RETENTION\_RATE} \\
\hline
...
Parameter atrisi (CT0) melengkapi wawasan dengan indikator retensi, memampukan para pengambil keputusan untuk merancang mitigasi kepergian pelanggan (\textit{churn}) dengan lebih efektif.

\subsection{Analisis Kualitas Data}
\label{subsec:kualitas-data}

Evaluasi kualitas data untuk peramalan deret waktu (\textit{time series forecasting}) meliputi tiga aspek utama, yaitu kelengkapan dan konsistensi data, struktur temporal, serta relevansi fitur. Proses evaluasi ini bertujuan untuk memastikan bahwa data yang digunakan memenuhi standar kualitas yang diperlukan untuk menghasilkan model peramalan yang akurat dan andal.

\subsubsection{Kelengkapan dan Konsistensi Data}

Pemeriksaan kelengkapan data dilakukan untuk mengidentifikasi nilai yang hilang (\textit{missing values}) atau ketidakkonsistenan dalam interval pengambilan sampel. Data deret waktu sangat bergantung pada observasi sekuensial sehingga \textit{timestamp} yang hilang atau interval pengambilan sampel yang tidak konsisten dapat menyebabkan asumsi model menjadi tidak valid.

Profiling dilakukan melalui kueri \texttt{SELECT} langsung ke skema \texttt{DWH\_MOIS} pada \textit{Enterprise Data Warehouse} selama periode pelatihan model (Mei--Juni 2026). Setiap metrik diagregasi pada level bulanan sehingga satu baris mewakili satu bulan observasi. Proses penarikan data menerapkan filter \texttt{IS NOT NULL} pada kolom kunci masing-masing tabel guna memastikan hanya baris yang memiliki nilai valid yang diproses. Tabel~\ref{tab:kualitas-data} menyajikan hasil profiling nyata berdasarkan jumlah titik data (\textit{data points}) bulanan yang berhasil ditarik untuk setiap metrik pada level nasional.

\begin{table}[htbp]
\centering
\caption{Hasil Profiling Data Deret Waktu per Metrik (Level Nasional)}
\label{tab:kualitas-data}
\begin{tabular}{|p{3.4cm}|p{3.2cm}|p{2.4cm}|p{2cm}|p{2.4cm}|}
\hline
\textbf{Metrik} & \textbf{Tabel Sumber} & \textbf{Titik Data (Bulan)} & \textbf{Rentang} & \textbf{Filter \texttt{NULL}} \\
\hline
\texttt{order\_hsi} & \texttt{BRIGHTAI\_SALES} & 55 & 41--55 & \texttt{ORDER\_DATE IS NOT NULL} \\
\hline
\texttt{fulfillment\_rate} & \texttt{BRIGHTAI\_SALES} & 55 & 41--55 & \texttt{ORDER\_DATE IS NOT NULL} \\
\hline
\texttt{avg\_install\_days} & \texttt{BRIGHTAI\_SALES} & 50 & 41--50 & \texttt{ORDER\_DATE}, \texttt{TGL\_PS IS NOT NULL} \\
\hline
\texttt{churn\_hsi} & \texttt{BRIGHTAI\_CT0\_NAL} & 40 & 20--40 & \texttt{PERIODE IS NOT NULL} \\
\hline
\texttt{realisasi\_hsi} & \texttt{BRIGHTAI\_TARGET} & 25 & 24--25 & \texttt{REALISASI}, \texttt{TREG IS NOT NULL} \\
\hline
\end{tabular}
\end{table}

Kolom \textit{Rentang} menunjukkan variasi jumlah titik data pada level regional dan witel karena beberapa wilayah memiliki data historis yang lebih pendek. Secara keseluruhan, 185 seri waktu dilatih per model (5 metrik $\times$ 37 entitas: 1 nasional + 5 regional + 31 witel), dengan 169 seri (91{,}4\%) memenuhi ambang batas kelulusan KPI\@. Metrik \texttt{churn\_hsi} dan \texttt{realisasi\_hsi} memiliki titik data yang lebih sedikit karena tabel sumber mulai dipopulasi pada periode yang lebih akhir dibandingkan tabel \texttt{BRIGHTAI\_SALES}.
...
\hline
Hierarki Wilayah & \texttt{REGIONAL}, \texttt{WITEL}, \texttt{DATEL}, \texttt{STO}, \texttt{CSTO} \\
\hline
...
Inventarisasi kualitas di atas menunjukkan bahwa meskipun sebagian besar atribut memiliki keutuhan informasi yang layak, beberapa anomali memerlukan prapemrosesan agar terhindar dari distorsi kalkulasi analitik.
...
\hline
Tingkat \textit{Churn} & Normalisasi Min-Max & Nilai terbatas pada rentang 0--1 \\
\hline
...
Setelah langkah pembersihan ini, keandalan kumpulan data dapat dijamin, menihilkan peluang munculnya duplikasi sinyal yang merugikan akurasi inferensi di tahap berikutnya.
...
Transformasi logaritmik ini secara matematis meredam rentang deviasi (\textit{variance}) deret waktu yang bersifat eksponensial sehingga data berada pada distribusi berskala lebih seragam dan mempermudah konvergensi model komputasi.
...
Konstruksi data temporal di atas menyediakan konteks kronologis fundamental untuk memosisikan setiap observasi data dalam aliran peristiwa yang berkesinambungan.
...
Variabel-variabel retrospektif ini memainkan peran krusial dalam mengungkap pola autokorelasi, dengan performa pada bulan-bulan sebelumnya berfungsi sebagai prediktor kuat bagi tren di periode yang akan datang.
...
Sebagai pelengkap aspek waktu, agregasi lintas entitas (nasional maupun regional) memberikan kerangka referensi hierarkis yang memampukan model untuk mengenali dinamika proporsional wilayah tertentu terhadap konstelasi bisnis secara keseluruhan.
...
Tahap pemodelan data (\textit{modeling}) merupakan fase keempat dalam metodologi CRISP-DM yang berfokus pada pemilihan teknik pemodelan, perancangan arsitektur model, konfigurasi \textit{hyperparameter}, serta pelatihan model menggunakan dataset yang telah dipersiapkan. Pada penelitian ini, pemodelan data mencakup tiga komponen utama, yaitu model peramalan deret waktu berbasis pembelajaran mendalam, model klasifikasi \textit{intent} berbasis pola, dan model pembangkitan bahasa alami berbasis templat.

\subsection{Arsitektur Model Peramalan Deret Waktu}
\label{subsec:arsitektur-forecasting}

Sistem peramalan dalam penelitian ini mengadopsi pendekatan bertahap yang dimulai dari model statistik sebagai \textit{baseline}, kemudian dibandingkan dengan model berbasis pembelajaran mendalam (\textit{deep learning}). Evaluasi perbandingan ini bertujuan untuk memastikan bahwa kompleksitas tambahan model \textit{deep learning} memberikan peningkatan akurasi yang signifikan dan terukur. Sistem menggunakan empat model yang dikategorikan sebagai berikut: (1) model statistik \textit{baseline} berupa ARIMA dan Prophet, serta (2) model \textit{deep learning} utama berupa LSTM (\textit{Long Short-Term Memory}) dan GRU (\textit{Gated Recurrent Unit}).

\subsubsection{Model \textit{Baseline} Statistik: ARIMA dan Prophet}

Model ARIMA (\textit{Autoregressive Integrated Moving Average}) dan Prophet digunakan sebagai \textit{baseline} pembanding dalam evaluasi sistem peramalan. Penggunaan \textit{baseline} statistik ini penting untuk memberikan konteks kuantitatif bagi penilaian performa model \textit{deep learning}: apabila model \textit{deep learning} tidak secara konsisten mengungguli \textit{baseline} statistik, maka penggunaan model yang lebih kompleks tidak dapat dibenarkan secara teknis dan praktis.

ARIMA diimplementasikan untuk setiap seri waktu secara individual menggunakan seleksi parameter otomatis berdasarkan kriteria informasi AIC (\textit{Akaike Information Criterion}). Data yang menunjukkan pola musiman yang signifikan secara statistik (berdasarkan uji Augmented Dickey-Fuller) menggunakan varian SARIMA dengan parameter musiman (\(P, D, Q, m\)).

Prophet diimplementasikan dengan konfigurasi standar yang mencakup deteksi titik perubahan tren secara otomatis dan komponen musiman tahunan. Prophet dipilih sebagai \textit{baseline} kedua karena kemampuannya menangani data bisnis dengan tren dan musiman yang kompleks tanpa memerlukan pra-pemrosesan manual yang ekstensif.

Hasil evaluasi \textit{baseline} statistik ini kemudian dibandingkan dengan model \textit{deep learning} menggunakan metrik sMAPE, MASE, dan \textit{Skill Score} pada protokol \textit{walk-forward validation} yang sama, untuk memastikan perbandingan yang adil dan representatif.
...
GRU merupakan varian yang lebih sederhana dari LSTM dengan jumlah parameter yang lebih sedikit sehingga lebih efisien dalam menangkap dan mempertahankan dependensi jangka pendek dan jangka panjang dalam data deret waktu. GRU menggunakan dua gerbang utama, yaitu \textit{update gate} dan \textit{reset gate}. Tabel~\ref{tab:gru-gates} menyajikan komponen gerbang pada arsitektur GRU.
...
Tabel~\ref{tab:perbandingan-model} menyajikan perbandingan karakteristik seluruh model peramalan yang diimplementasikan dalam penelitian ini, mencakup model statistik \textit{baseline} (ARIMA/SARIMA dan Prophet) serta model \textit{deep learning} utama (LSTM dan GRU).
...
\hline
\textbf{Aspek Perbandingan} & \textbf{ARIMA} & \textbf{Prophet} & \textbf{LSTM} & \textbf{GRU} \\
\hline
Kompleksitas Arsitektur & Rendah & Rendah & Tinggi & Sedang \\
\hline
Jumlah Parameter & Sedikit (\(p,d,q\)) & Sedikit & Banyak & Lebih sedikit \\
\hline
Kecepatan Pelatihan & Sangat Cepat & Cepat & Lambat & Cepat \\
\hline
Dependensi Jangka Panjang & Terbatas & Sedang & Baik & Baik \\
\hline
Kemampuan Nonlinear & Tidak & Sebagian & Baik & Baik \\
\hline
Interpretabilitas & Sangat Tinggi & Tinggi & Rendah & Rendah \\
\hline
Kemampuan Multivariat & Terbatas & Terbatas & Baik & Baik \\
\hline
Kebutuhan Volume Data & Rendah & Rendah & Sedang & Sedang \\
\hline
Peran dalam Penelitian & \textit{Baseline} & \textit{Baseline} & Model Utama & Model Utama \\
\hline
\end{tabular}
\end{table}

Pemisahan partisi antara porsi historis, periode validasi, dan pengujian pamungkas mengamankan protokol pengujian yang objektif, mencegah terjadinya asimilasi informasi (\textit{data leakage}) yang sering menjadi jebakan pemodelan deret waktu.

\subsection{Konfigurasi \textit{Hyperparameter}}
\label{subsec:hyperparameter}

Konfigurasi \textit{hyperparameter} untuk model LSTM dan GRU ditetapkan sebagai nilai tetap pada berkas \texttt{config.py} berdasarkan praktik terbaik dari literatur. Tabel~\ref{tab:hyperparameter} menyajikan konfigurasi \textit{hyperparameter} yang digunakan dalam pelatihan.

\begin{table}[htbp]
\centering
\caption{Konfigurasi \textit{Hyperparameter} Model (Berkas \texttt{config.py})}
\label{tab:hyperparameter}
\begin{tabular}{|p{4.5cm}|c|c|p{4.5cm}|}
\hline
\textbf{\textit{Hyperparameter}} & \textbf{LSTM} & \textbf{GRU} & \textbf{Keterangan} \\
\hline
Jumlah Lapisan Berulang (\texttt{NUM\_LAYERS}) & 2 & 2 & Kedalaman arsitektur \textit{recurrent} \\
\hline
Ukuran \textit{Hidden State} (\texttt{HIDDEN\_SIZE}) & 64 & 64 & Dimensi representasi internal \\
\hline
Tingkat \textit{Dropout} (\texttt{DROPOUT}) & 0{,}2 & 0{,}2 & Regularisasi antar-lapisan \\
\hline
\textit{Learning Rate} (\texttt{LEARNING\_RATE}) & $10^{-3}$ & $10^{-3}$ & Laju pembelajaran Adam \\
\hline
Ukuran \textit{Batch} (\texttt{BATCH\_SIZE}) & 32 & 32 & Jumlah sampel per iterasi \\
\hline
Jumlah \textit{Epoch} Maksimum (\texttt{EPOCHS}) & 100 & 100 & Batas iterasi pelatihan \\
\hline
\textit{Early Stopping Patience} & 20 & 20 & \textit{Epoch} tanpa perbaikan sebelum berhenti \\
\hline
Panjang Sekuens Masukan (\texttt{WINDOW\_SIZE}) & 12 & 12 & Jumlah bulan historis sebagai input \\
\hline
Horizon Prediksi (\texttt{HORIZON}) & 1 & 1 & Jumlah bulan yang diprediksi \\
\hline
\end{tabular}
\end{table}

Matriks nilai di atas mendefinisikan ruang pencarian (\textit{search space}) algoritma untuk menemukan takaran interaksi parameter yang optimal, menyeimbangkan kompleksitas komputasional dengan kapasitas generalisasi masing-masing model.
...
\hline
Prediksi Titik Numerik & \textit{Mean Squared Error} (MSE) & Mengukur rata-rata kuadrat kesalahan prediksi untuk nilai kontinu, termasuk seluruh metrik operasional (volume pesanan, jumlah \textit{churn}, pendapatan, dan lainnya) \\
\hline
...
Perlu ditekankan bahwa seluruh metrik yang diramalkan dalam sistem ini, termasuk \textit{churn}, diperlakukan sebagai tugas peramalan deret waktu numerik (\textit{numeric time series forecasting}), bukan sebagai tugas klasifikasi biner. Metrik \texttt{churn\_hsi} merepresentasikan jumlah agregat pelanggan yang berhenti berlangganan (\texttt{COUNT(DISTINCT NCLI)}) pada level bulanan per wilayah sehingga fungsi kerugian yang digunakan adalah MSE, bukan \textit{Binary Cross-Entropy}. Prediksi \textit{churn} biner per pelanggan individual (apakah seorang pelanggan tertentu akan \textit{churn} atau tidak) merupakan tugas klasifikasi yang memerlukan protokol evaluasi terpisah (akurasi, presisi, daya ingat, AUC-ROC) dan tidak diimplementasikan pada versi sistem ini.

\subsection{Model Klasifikasi \textit{Intent} Berbasis Pola}
\label{subsec:model-intent}

Pencocokan kueri pengguna ke aturan bisnis menggunakan pendekatan pencocokan pola (\textit{pattern matching}) yang dikombinasikan dengan mekanisme \textit{confidence scoring}. Sistem mengimplementasikan tujuh kategori \textit{intent} (\textit{quantity}, \textit{trend}, \textit{comparison}, \textit{location}, \textit{performance}, \textit{detail}, \textit{reason}) beserta satu \textit{fallback} (\textit{general}), yang dikombinasikan dengan delapan kategori fokus analisis untuk dipetakan ke 38 aturan bisnis pada lima domain data.

Dalam setiap \textit{intent}, didefinisikan sekumpulan pola (\textit{pattern}) yang mencakup variasi pertanyaan yang mungkin diajukan pengguna. Sistem menghitung skor kecocokan (\textit{matching score}) antara kueri pengguna dengan setiap pola dan menghasilkan \textit{confidence score} dalam rentang 0--100 melalui formula $c = \min(100,\;\text{skor} \times 10)$. Ambang batas (\textit{threshold}) ditetapkan pada nilai 40 (setara 0{,}40 pada skala 0--1); kueri dengan \textit{confidence score} di atas ambang batas diteruskan ke modul pemrosesan kueri, sedangkan kueri di bawah ambang batas memicu permintaan klarifikasi kepada pengguna.
...
\hline
Klasifikasi \textit{Intent} & \textit{Pattern Matching} + \textit{Confidence Scoring} & Kueri teks pengguna & \textit{Intent}, entitas, confidence score \\
\hline
Peramalan Deret Waktu & LSTM / GRU (ARIMA \& Prophet sebagai \textit{baseline}) & Data historis bulanan & Prediksi nilai metrik \\
\hline
Pembangkitan Respons & \textit{Template-based} NLG & Data hasil kueri / prediksi & Teks naratif bahasa Indonesia \\
\hline
\end{tabular}
\end{table}
