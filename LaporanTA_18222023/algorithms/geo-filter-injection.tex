...
\caption{Injeksi Filter Geografis ke dalam Kueri SQL (Bagian 1)}
\label{alg:geo-filter-1}
\begin{algorithmic}[1]
...
Bagian pertama algoritma di atas mendemonstrasikan proses deteksi cakupan geografis dari teks masukan pengguna. Sistem melakukan pencocokan terhadap daftar referensi regional dan \textit{witel} untuk mengonstruksi klausa filter secara dinamis. 

Setelah klausa filter berhasil dibentuk, tahap berikutnya adalah mengimplementasikan penyisipan sintaks tersebut ke dalam badan SQL asli, yang diuraikan pada bagian kedua algoritma berikut ini.

\begin{algorithm}[htbp]
\caption{Injeksi Filter Geografis ke dalam Kueri SQL (Bagian 2)}
\label{alg:geo-filter-2}
\begin{algorithmic}[1]
\algrestore{geofilter}
