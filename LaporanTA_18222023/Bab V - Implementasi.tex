...
\begin{enumerate}
    \item \textit{Frontend (React.js)}: Antarmuka pengguna berbasis web yang menyediakan fitur percakapan \textit{chatbot}, panel peramalan, dan manajemen profil pengguna. \textit{Frontend} berjalan pada porta 3000 dan berkomunikasi dengan \textit{backend} melalui \textit{RESTful API}.
    \item \textit{Backend (Node.js/Express)}: Peladen utama yang menangani autentikasi, manajemen sesi, eksekusi mesin aturan (\textit{rule engine}), klasifikasi maksud (\textit{intent classification}), pembangkitan bahasa alami (NLG), serta komunikasi dengan basis data Oracle. \textit{Backend} berjalan pada porta 3001.
    \item Layanan \textit{Machine Learning (FastAPI/PyTorch)}: Layanan terpisah yang menangani pelatihan dan inferensi model peramalan deret waktu berbasis GRU dan LSTM. Layanan ini berjalan pada porta 8000 dan diakses oleh \textit{backend} melalui permintaan HTTP internal.
\end{enumerate}

{\sloppy Ketiga lapis terhubung ke Oracle Database yang berfungsi ganda melalui dua skema terpisah: (1) skema \texttt{DWH\_MOIS} sebagai \textit{Enterprise Data Warehouse} (DWH)\label{abbr:DWH} yang menyimpan lima tabel domain analitik dengan prefiks \texttt{BRIGHTAI\_} (\texttt{BRIGHTAI\_}\allowbreak\texttt{SALES}, \allowbreak\texttt{BRIGHTAI\_}\allowbreak\texttt{REVENUE}, \allowbreak\texttt{BRIGHTAI\_}\allowbreak\texttt{TARGET}, \allowbreak\texttt{BRIGHTAI\_}\allowbreak\texttt{DAPROS}, \allowbreak\texttt{BRIGHTAI\_}\allowbreak\texttt{CT0\_}\allowbreak\texttt{NAL}); dan (2) skema \texttt{USR\_RPT} yang menyimpan data operasional sistem, meliputi tabel pengguna (\texttt{BRIGHTAI\_}\allowbreak\texttt{USER}), riwayat percakapan (\texttt{BRIGHTAI\_}\allowbreak\texttt{CHAT}), dan sesi (\texttt{BRIGHTAI\_}\allowbreak\texttt{SESSION}).\par}
...
\input{tables/tabel-struktur-backend.tex}
...
\begin{enumerate}
    \item Helmet: Mengatur tajuk HTTP keamanan (\textit{security headers}) untuk mencegah serangan seperti \textit{Cross-Site Scripting} (XSS) dan \textit{clickjacking}.
    \item CORS\label{abbr:CORS} (\textit{Cross-Origin Resource Sharing}): Mengizinkan permintaan lintas-asal dari \textit{frontend} yang berjalan pada domain berbeda.
    \item \textit{Rate Limiter}: Membatasi jumlah permintaan per alamat IP menjadi maksimal 100 permintaan per 15 menit untuk mencegah serangan \textit{brute force} dan \textit{Denial of Service} (DoS).
    \item Pembatasan Ukuran \textit{Payload}: Membatasi ukuran badan permintaan JSON menjadi maksimal 10 MB.
\end{enumerate}
...
\input{tables/tabel-api-routes.tex}
...
\begin{enumerate}
    \item Penerimaan Kueri: Modul \texttt{chatController.js} menerima pesan pengguna dan memeriksa apakah pesan tersebut merupakan sapaan, permintaan bantuan, atau kueri analitik.
    \item Pencocokan Aturan: Modul \texttt{queryProcessor.js} mencocokkan kueri pengguna dengan aturan yang tersedia menggunakan pencocokan pola kata kunci.
    \item Eksekusi Aturan: Modul \texttt{ruleEngine.js} mengeksekusi kueri SQL yang terkait dengan aturan yang cocok terhadap Oracle Database, dengan injeksi filter geografis secara dinamis.
    \item Pembangunan Respons: Modul \texttt{responseBuilder.js} dan \texttt{nlgGenerator.js} mengonversi hasil kueri menjadi narasi dalam bahasa Indonesia.
\end{enumerate}
...
Sistem mendukung 38 aturan analitik yang tersebar di lima tabel domain pada skema \texttt{DWH\_MOIS}, yaitu \texttt{BRIGHTAI\_}\allowbreak\texttt{SALES}, \allowbreak\texttt{BRIGHTAI\_}\allowbreak\texttt{REVENUE}, \allowbreak\texttt{BRIGHTAI\_}\allowbreak\texttt{TARGET}, \allowbreak\texttt{BRIGHTAI\_}\allowbreak\texttt{DAPROS}, dan \allowbreak\texttt{BRIGHTAI\_}\allowbreak\texttt{CT0\_}\allowbreak\texttt{NAL}. \textit{Listing}~\ref{lst:rule-definition} menyajikan contoh implementasi struktur definisi aturan secara lengkap.

\input{listings/rule-definition.tex}
...
\begin{enumerate}
    \item Mode Penampung (\textit{placeholder)}: Jika kueri SQL mengandung penanda \texttt{/* :GEO\_FILTER: */}, penanda tersebut diganti dengan klausa filter yang sesuai.
    \item Mode Injeksi Otomatis: Jika tidak ada penanda, sistem melakukan pemindaian baris demi baris untuk menemukan blok \texttt{FROM DWH\_MOIS} dan menyisipkan klausa filter pada posisi yang tepat sebelum \texttt{GROUP BY} atau penutup subkueri.
\end{enumerate}

Dalam membedah mekanisme tersebut, Algoritma~\ref{alg:geo-filter-1} merinci prosedur tahap awal yang mengekstraksi intensi cakupan geografis dari pertanyaan pengguna. Proses ini diawali dengan pengecekan hierarkis mulai dari kata kunci tingkat nasional hingga pencocokan struktur \textit{witel}.

\input{algorithms/geo-filter-injection.tex}

Eksekusi modifikasi SQL pada algoritma di atas merepresentasikan kemampuan sistem dalam menyisipkan klausa \texttt{WHERE} tambahan secara terprogram. Penggunaan pencarian \textit{placeholder} serta pemindaian leksikal mendalam memastikan integritas logika basis data tetap terjaga. Melalui arsitektur ini, \textit{dashboard} percakapan dijamin hanya menarik dan merender analisis agregat untuk geografi yang relevan dan memiliki hak akses, mewujudkan prinsip isolasi wilayah fungsional.
...
\input{tables/tabel-intent-impl.tex}

\subsection{Algoritma Perhitungan Skor}

Dalam setiap kueri, sistem menghitung skor kecocokan terhadap setiap \textit{intent} dengan mengiterasi daftar kata kunci. Jika sebuah kata kunci ditemukan dalam kueri (menggunakan fungsi \texttt{String.includes}), skor \textit{intent} tersebut ditambah sebesar nilai bobot. \textit{Intent} dengan skor tertinggi dipilih sebagai hasil klasifikasi, dengan nilai keyakinan (\textit{confidence}) dihitung sebagai $\min(100, \text{skor} \times 10)$\label{sym:min}. Jika seluruh skor bernilai nol, sistem mengembalikan \textit{intent} \texttt{general} sebagai \textit{fallback}.

Selain \textit{intent}, sistem juga mengklasifikasikan fokus analisis ke dalam delapan kategori: \textit{order}, \textit{revenue}, \textit{churn}, \textit{fulfillment}, \textit{subscriber}, \textit{bundling}, \textit{digital}, dan \textit{target}. Kombinasi \textit{intent} dan fokus analisis ini digunakan secara sinergis oleh modul pembangkitan bahasa alami (NLG) untuk memilih templat respons yang paling sesuai dengan konteks percakapan. \textit{Listing}~\ref{lst:query-processor} menunjukkan secara nyata implementasi inti dari proses pencocokan aturan pada kelas \textit{query processor}.

\input{listings/query-processor.tex}

Dalam mengikat seluruh alur komputasi di atas, diperlukan sebuah algoritma sekuensial yang merangkum keseluruhan mekanisme klasifikasi. Algoritma~\ref{alg:intent-classification} mendeskripsikan prosedur tersebut secara formal, mulai dari penerimaan masukan, iterasi pencocokan pola, hingga penentuan maksud dan fokus akhir.

\input{algorithms/intent-classification.tex}

Dengan adanya algoritma yang terstruktur ini, modul pemroses kueri dapat membedakan berbagai bentuk pertanyaan pengguna dan memetakannya secara akurat ke dalam format yang dimengerti oleh sistem basis data dan mesin analitik.
...
\begin{enumerate}
    \item Ekstraksi Konteks: Fungsi \texttt{extractCtx} mengekstraksi informasi kontekstual dari data terstruktur hasil kueri, meliputi periode analisis, nilai utama, pertumbuhan MoM/YoY, distribusi produk, metrik pelanggan, dan wawasan bisnis.
    \item Klasifikasi \textit{Intent}: Modul \texttt{intentClassifier} mengklasifikasikan \textit{intent} dan fokus dari pertanyaan asli pengguna.
    \item Pemilihan Templat: Fungsi \texttt{getTemplate} dari \texttt{templateLibrary} memilih templat yang sesuai berdasarkan \textit{intent} yang teridentifikasi. Setiap \textit{intent} memiliki minimal tiga variasi templat.
    \item Pengisian Data: Templat yang terpilih diisi dengan data faktual dari konteks. Seluruh angka diformat menggunakan \textit{locale} Indonesia (\texttt{id-ID}).
\end{enumerate}
...
Kode templat di atas memastikan keseragaman penyajian laporan bisnis, dengan format respons telah dibakukan agar selaras dengan kebutuhan baca pengguna operasional di berbagai domain.
...
Akurasi faktual dijamin sebesar 100\% karena seluruh angka dalam respons berasal langsung dari hasil kueri Oracle Database tanpa melalui proses halusinasi teks generatif. \textit{Listing}~\ref{lst:nlg-generator} secara teknis menyajikan implementasi kelas dan fungsi utama yang bertanggung jawab merajut data basis data menjadi laporan \textit{Markdown} yang komprehensif.

\input{listings/nlg-generator.tex}

Dalam memberikan gambaran operasional yang holistik, prosedur dari pembentukan konteks hingga pemilihan frasa dijelaskan secara formal melalui sebuah algoritma deterministik. Algoritma~\ref{alg:nlg-template} mendeskripsikan prosedur lengkap pembangkitan respons mulai dari ekstraksi konteks terstruktur hingga produksi narasi akhir berbahasa Indonesia.

\input{algorithms/nlg-template-selection.tex}

Dengan penerapan algoritma tersebut, sistem terbukti mampu menyajikan narasi bisnis yang bervariasi tanpa mengorbankan akurasi data analitik maupun konsistensi format pelaporan.
...
Struktur \textit{endpoints} yang tersusun rapi tersebut tidak hanya menjamin abstraksi layanan, tetapi juga memisahkan tanggung jawab antara lapisan aplikasi antarmuka dan lapisan komputasi analitik, sejalan dengan prinsip mikroservis.
...
\begin{enumerate}
    \item Lapisan Berulang: GRU menggunakan \texttt{nn.GRU} dan LSTM menggunakan \texttt{nn.LSTM}, masing-masing dengan 2 lapisan bertumpuk (\textit{stacked layers}) dan ukuran \textit{hidden state} sebesar 64 unit.
    \item Lapisan \textit{Dropout}: Regularisasi dengan tingkat \textit{dropout} 0,2 diterapkan setelah lapisan berulang untuk mencegah \textit{overfitting}.
    \item Lapisan Linear Keluaran: Lapisan \texttt{nn.Linear} yang memetakan \textit{hidden state} terakhir ke dimensi horizon prediksi.
\end{enumerate}

Kedua model dirancang untuk menerima matriks masukan berdimensi (\textit{batch}, \texttt{window\_}\allowbreak\texttt{size}, 1) dan mereduksinya menjadi representasi peramalan berukuran (\textit{batch}, \textit{horizon}). Sebagai representasi pertama dari rancangan tersebut, \textit{Listing}~\ref{lst:gru-model} menyajikan implementasi kode dari arsitektur GRU. Komponen ini menitikberatkan pada efisiensi komputasi dengan memanfaatkan gerbang pembaruan (\textit{update gate}) dan gerbang atur ulang (\textit{reset gate}) secara terintegrasi untuk mengendalikan aliran memori sekuensial.

\input{listings/gru-model.tex}

Berbeda dengan kesederhanaan struktural yang ditawarkan oleh mekanisme pembaruan GRU, arsitektur LSTM menghadirkan tingkat kendali memori yang lebih presisi melalui tambahan status sel (\textit{cell state}) yang beroperasi secara berdampingan dengan status tersembunyi (\textit{hidden state}). Konstruksi lapisan ini secara spesifik diuraikan pada \textit{Listing}~\ref{lst:lstm-model}, menunjukkan inisialisasi memori ganda yang krusial untuk mitigasi peluruhan gradien (\textit{vanishing gradient}) pada deret waktu berdurasi panjang.

\input{listings/lstm-model.tex}

Selain arsitektur dasar, kinerja model sangat dipengaruhi oleh konfigurasi \textit{hyperparameter} yang ditetapkan sebelum proses pelatihan dimulai. Konfigurasi bawaan yang diterapkan dalam layanan ini disajikan secara perinci pada Tabel~\ref{tab:hyperparameter-impl}. Pengaturan tersebut dirancang untuk mencapai keseimbangan optimal antara kapasitas pembelajaran model dan risiko terjadinya \textit{overfitting}.
...
Setelah seluruh konfigurasi ditetapkan, model dilatih menggunakan algoritma pembelajaran standar. Algoritma~\ref{alg:early-stopping} mendeskripsikan prosedur pelatihan GRU dan LSTM secara formal, termasuk integrasi mekanisme penghentian dini (\textit{early stopping}) dan penjadwalan laju pembelajaran (\textit{learning rate scheduling}). Langkah-langkah tersebut memastikan bahwa pelatihan akan dihentikan secara otomatis apabila model telah mencapai titik konvergensi.

\input{algorithms/early-stopping.tex}

Penerapan algoritma di atas secara efektif memastikan efisiensi komputasi, mencegah penggunaan sumber daya yang berlebihan, serta mengamankan model akhir agar memiliki tingkat generalisasi terbaik terhadap data validasi.
...
Pemilihan ukuran jendela historis (\textit{window size}) merupakan faktor krusial dalam peramalan deret waktu. Ketika pengguna tidak menentukan ukuran jendela secara spesifik (\texttt{window\_size = 0}), sistem akan mengevaluasi sejumlah kandidat, yaitu 3, 6, 9, dan 12 bulan. Kandidat tersebut dibatasi hingga maksimal 50\% dari total data pra-pengujian untuk memastikan model memiliki cukup data observasi laten. Algoritma~\ref{alg:auto-tune-window} mendeskripsikan prosedur pemilihan ukuran jendela optimal tersebut secara formal.

\input{algorithms/auto-tune-window.tex}

Melalui pendekatan adaptif ini, sistem dapat mengakomodasi berbagai karakteristik deret waktu yang berbeda pada setiap wilayah atau layanan sehingga proses penyetelan parameter secara manual yang memakan waktu lama tidak lagi diperlukan.

\subsubsection{Validasi \textit{Walk-Forward}}

Teknik \textit{k-fold cross-validation} standar tidak dapat digunakan untuk evaluasi kinerja model karena teknik tersebut mengabaikan urutan temporal dari data deret waktu. Sebagai gantinya, evaluasi model ini menggunakan protokol \textit{walk-forward va\-li\-da\-tion} yang merupakan validasi silang khusus deret waktu. Pada setiap tahapan iterasi, model dilatih secara kumulatif pada seluruh data historis hingga bulan ke-$t$, lalu digunakan untuk memprediksi data bulan ke-$t+1$. Proses pengujian ini diulang untuk $n$ bulan terakhir (minimum 3 bulan, maksimum 6 bulan) guna menghasilkan estimasi metrik evaluasi yang jauh lebih realistis dalam skenario produksi. Algoritma~\ref{alg:walk-forward} mendeskripsikan prosedur operasional dari \textit{walk-forward va\-li\-da\-tion} tersebut secara formal.

\input{algorithms/binary-search-alg.tex}

Implementasi dari prosedur validasi tersebut menjamin bahwa metrik kesalahan seperti MAE, RMSE, sMAPE, MASE, dan \textit{Skill Score} tidak bias terhadap bocoran data masa depan (\textit{data leakage}) sehingga merepresentasikan ketangguhan model yang sebenarnya.
...
Dalam mendukung reprodusibilitas hasil eksperimen, sistem mengimplementasikan beberapa mekanisme berikut.

\begin{enumerate}
    \item Penyimpanan artefak model: Setiap model terbaik disimpan sebagai berkas \texttt{.pth} (PyTorch) atau \texttt{.pkl} (ARIMA/\textit{Prophet}) di direktori \texttt{saved\_models/}. Berkas \textit{checkpoint} menyimpan tidak hanya \texttt{state\_dict} model, tetapi juga seluruh \textit{hyperparameter} yang digunakan (\texttt{window\_}\allowbreak\texttt{size}, \allowbreak\texttt{hidden\_}\allowbreak\texttt{size}, \allowbreak\texttt{num\_}\allowbreak\texttt{layers}, \allowbreak\texttt{dropout}), jenis model, metrik, regional, dan \textit{witel}. Parameter normalisasi (\texttt{MinMaxScaler}) disimpan terpisah dalam berkas \texttt{.pkl} sehingga inferensi dapat menggunakan \textit{scaler} yang identik dengan saat pelatihan.

    \item Pelacakan riwayat pelatihan: Seluruh hasil pelatihan dicatat dalam berkas \texttt{training\_history.json} yang menyimpan metadata lengkap setiap eksperimen, meliputi tipe model, metrik target, cakupan geografis, jumlah titik data, ukuran jendela, metode evaluasi (\textit{walk-forward}), seluruh metrik evaluasi (MAE, RMSE, sMAPE, MASE, \textit{Skill Score}), hasil \textit{benchmark} terhadap model sebelumnya, serta stempel waktu pelatihan.

    \item Versi pustaka terkunci: Berkas \texttt{requirements.txt} menetapkan versi eksak untuk setiap dependensi (misalnya \texttt{torch==2.1.1}, \allowbreak\texttt{fastapi==0.104.1}, \allowbreak\texttt{scikit-}\allowbreak\texttt{learn==1.3.2}) sehingga lingkungan pelatihan dapat direkonstruksi secara konsisten.

    \item Pencegahan regresi: Mekanisme \textit{benchmark} (Subbab~\ref{subsec:pipeline-pelatihan}) memastikan bahwa model baru hanya menggantikan model lama jika memiliki sMAPE lebih rendah atau \textit{Skill Score} lebih tinggi sehingga setiap artefak yang tersimpan merupakan model terbaik yang pernah dilatih untuk kombinasi metrik dan wilayah tertentu.
\end{enumerate}

\paragraph{Keterbatasan.} Implementasi saat ini belum menetapkan \textit{random seed} secara eksplisit (misalnya \texttt{torch.}\allowbreak\texttt{manual\_}\allowbreak\texttt{seed}) sehingga hasil pelatihan tidak dapat direproduksi secara numerik identik di antara dua sesi pelatihan berbeda. Namun, reprodusibilitas fungsional tetap terjaga melalui penyimpanan artefak model terbaik dan riwayat pelatihan yang lengkap sehingga model terlatih dapat dimuat ulang dan menghasilkan inferensi yang deterministik.

\subsection{Praolah Data}
\label{subsec:praolah-data}

Praolah data diimplementasikan dalam modul \texttt{utils/}\allowbreak\texttt{preprocessing.py} dengan tahapan berikut.

\begin{enumerate}
    \item Normalisasi: Data dinormalisasi menggunakan \texttt{MinMaxScaler} dari Scikit-learn ke rentang [0, 1]. Parameter \textit{scaler} disimpan ke berkas \texttt{.pkl} untuk digunakan kembali saat inferensi.
    \item Pembuatan \textit{Sliding Window}: Data yang telah dinormalisasi diubah menjadi pasangan masukan-keluaran (\textit{X}, \textit{y}) menggunakan jendela geser (\textit{sliding window}). Masukan \textit{X} berukuran (\texttt{n\_samples}, \texttt{window\_size}, 1) dan keluaran \textit{y} berukuran (\texttt{n\_samples}, \textit{horizon}).
    \item Pembagian Data: Data dibagi menjadi \textit{training set} (80\%) dan \textit{validation set} (20\%) secara kronologis tanpa pengacakan untuk mempertahankan urutan temporal.
\end{enumerate}
...
Implementasi metrik dengan konfigurasi *threshold* KPI secara langsung pada tahap kalkulasi ini memungkinkan komponen analitik sistem untuk mengategorikan model seketika menjadi tiga status operasional: LULUS, Lulus Dengan Catatan (LDC), atau GAGAL. Integrasi evaluasi bawaan ini menghilangkan kebutuhan akan pemrosesan data pasca-prediksi secara manual dan mengotomatiskan seleksi pemodelan *forecasting*.
...
\hline
\texttt{ForecastPanel.jsx} & Panel peramalan: pemilihan metrik, model, cakupan wilayah, dan penjelasan hasil prediksi \\
\hline
...
\begin{enumerate}
    \item Registrasi: Kata sandi pengguna di-\textit{hash} menggunakan algoritma \textit{bcrypt} dengan 10 putaran \textit{salt} sebelum disimpan ke basis data.
    \item Masuk (\textit{Login)}: Kredensial divalidasi terhadap basis data. Jika valid, peladen menerbitkan token JWT dengan masa berlaku 24 jam dan penerbit (\textit{issuer}) ``brightai-backend''.
    \item Otorisasi: Setiap permintaan ke rute yang dilindungi harus menyertakan token JWT pada tajuk \texttt{Authorization}. \textit{Middleware} autentikasi memverifikasi dan mendekode token untuk mengidentifikasi pengguna.
\end{enumerate}

\subsection{Validasi Masukan}

Validasi masukan pengguna diimplementasikan menggunakan pustaka \texttt{express-}\allowbreak\texttt{validator} dengan aturan yang mencakup validasi format surel, panjang minimal kata sandi (6 karakter), keharusan kombinasi huruf besar, huruf kecil, dan angka pada kata sandi, serta sanitasi terhadap serangan injeksi.
...
Antarmuka ini dirancang secara minimalis dengan memusatkan form masukan di tengah layar untuk mengurangi disonansi kognitif, mengizinkan pengguna untuk langsung berfokus pada autentikasi dasar yang aman.
...
Serupa dengan antarmuka \textit{login}, halaman registrasi memastikan konsistensi visual sekaligus menyediakan fungsionalitas pembuatan kredensial baru sebelum pengguna dapat mengakses kapabilitas kecerdasan buatan sistem.
...
Sebagai pusat kendali utama (\textit{centerpiece}) aplikasi, halaman ini mendedikasikan sebagian besar area layar untuk riwayat percakapan. Susunan pesan yang asinkron menegaskan sifat percakapan berbalas dengan jawaban sistem dirender dengan membedakan warna kotak teks.
...
Halaman profil memungkinkan pengguna mempersonalisasikan konfigurasi pribadinya, yang pada saat ini menampilkan rangkuman data personal sebagai konfirmasi akses sesi pengguna yang sedang aktif.
...
Aksesibilitas sistem turut disematkan melalui preferensi tema antarmuka pada halaman ini, mengizinkan penyesuaian kontras demi kenyamanan pemakaian jangka panjang dalam kondisi iluminasi lingkungan yang berbeda-beda.
...
Penggunaan \textit{light mode} sangat cocok bagi pengguna yang lebih menyukai tata ruang dokumen putih standar, dengan seluruh komponen \textit{React} diwarnai ulang melalui kelas utilitas \textit{Tailwind} (\textit{dark mode inversion}).
