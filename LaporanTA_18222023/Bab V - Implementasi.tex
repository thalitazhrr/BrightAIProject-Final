...
Ketiga lapis terhubung ke Oracle Database yang berfungsi sebagai \textit{Enterprise Data Warehouse} (DWH)\label{abbr:DWH} sekaligus penyimpan data operasional sistem (pengguna, riwayat percakapan, dan sesi).
...
\hline
Koneksi basis data & Oracle Database, \textit{connection pool} & Terimplementasi \\
\hline
\end{tabular}
\end{table}

Secara keseluruhan, sistem BrightAI telah diimplementasikan sebagai aplikasi tiga lapis yang mencakup: (1) \textit{frontend} berbasis React.js pada porta 3000 yang menyediakan antarmuka percakapan, panel peramalan interaktif, dan alur terpandu; (2) \textit{backend} berbasis Node.js/Express pada porta 3001 yang mengelola autentikasi, mesin aturan dengan 38 aturan analitik pada 5 domain, klasifikasi maksud, dan pembangkitan bahasa alami; serta (3) layanan \textit{machine learning} berbasis FastAPI/PyTorch pada porta 8000 yang menyediakan kemampuan peramalan menggunakan empat model, yaitu GRU, LSTM, ARIMA, dan \textit{Prophet}.

Seluruh komponen terhubung ke Oracle Database sebagai \textit{Enterprise Data Warehouse} dan menggunakan mekanisme \textit{caching} memori untuk mengoptimalkan waktu respons. Injeksi filter geografis secara dinamis memungkinkan sistem menyajikan analisis pada tingkat nasional, regional, maupun \textit{witel} tanpa duplikasi definisi aturan.
